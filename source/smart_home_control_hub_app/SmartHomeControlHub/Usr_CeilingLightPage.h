/*
This file is automatically generated
DO NOT MODIFY BY HAND
Usr_CeilingLightPage
Header
*/

#ifndef Usr_CeilingLightPage__H
#define Usr_CeilingLightPage__H

#include "CeilingLigehtCase2.h"
#include "CeilingLightWire1.h"
#include "CelingLight1.h"
#include "Esd_Base.h"
#include "Esd_Core.h"
#include "Esd_FontInfo.h"
#include "Ft_Esd.h"
#include "Ft_Esd_BitmapInfo.h"
#include "Ft_Esd_Gradient.h"
#include "Ft_Esd_Image.h"
#include "Ft_Esd_ImageButton.h"
#include "Ft_Esd_Label.h"
#include "Ft_Esd_Layout_Fixed.h"
#include "Ft_Esd_RangeSlider.h"
#include "Ft_Esd_Rectangle.h"
#include "Ft_Esd_Theme.h"
#include "Ft_Esd_Theme_DarkOrange.h"
#include "Ft_Esd_Widget.h"
#include "power_circle_64.h"
#include "return_circle_64.h"

#ifndef ESD_LOGIC
#define ESD_LOGIC(name, ...)
#define ESD_ACTOR(name, ...)
#define ESD_WIDGET(name, ...)
#define ESD_PAGE(name, ...)
#define ESD_APPLICATION(name, ...)
#define ESD_INPUT(name, ...)
#define ESD_ARRAY_INPUT(name, ...)
#define ESD_OUTPUT(name, ...)
#define ESD_SLOT(name, ...)
#define ESD_SIGNAL(name, ...)
#define ESD_VARIABLE(name, ...)
#define ESD_WRITER(name, ...)
#endif

#define Usr_CeilingLightPage_CLASSID 0x93301355
ESD_SYMBOL(Usr_CeilingLightPage_CLASSID, Type = esd_classid_t)

ESD_WIDGET(Usr_CeilingLightPage, Include = "Usr_CeilingLightPage.h", Callback, X = 0, Y = 0, Width = 400, Height = 300, Page)
typedef struct
{
	union
	{
		void *Owner;
		Ft_Esd_Widget Widget;
	};
	ESD_VARIABLE(SliderValue, Type = int, Default = 255, Private)
	int SliderValue;
	ESD_VARIABLE(LampColor, Type = esd_argb32_t, Default = #ffaa00, Private)
	esd_argb32_t LampColor;
	ESD_VARIABLE(On, Type = bool, Default = true, Private)
	bool On;
	ESD_SIGNAL(BackPushed)
	void(* BackPushed)(void *context);
	Ft_Esd_Layout_Fixed Fixed_Position;
	Ft_Esd_Image ESD_Image_4;
	Ft_Esd_Image ESD_Image_3;
	Ft_Esd_RangeSlider ESD_RangeSlider;
	Ft_Esd_Label ESD_Label;
	Ft_Esd_ImageButton ESD_Image_Button_2;
	Ft_Esd_ImageButton ESD_Image_Button;
	Ft_Esd_Image ESD_Image_2;
	Ft_Esd_Rectangle ESD_Rectangle_Widget_4;
	Ft_Esd_Gradient ESD_Gradient_Widget;
} Usr_CeilingLightPage;

void Usr_CeilingLightPage__Initializer(Usr_CeilingLightPage *context);

ESD_SLOT(Update)
void Usr_CeilingLightPage_Update(Usr_CeilingLightPage *context);

ESD_SLOT(End)
void Usr_CeilingLightPage_End(Usr_CeilingLightPage *context);

ESD_SLOT(Start)
void Usr_CeilingLightPage_Start(Usr_CeilingLightPage *context);

ESD_SLOT(Right)
void Usr_CeilingLightPage_Right(Usr_CeilingLightPage *context);

ESD_SLOT(Left)
void Usr_CeilingLightPage_Left(Usr_CeilingLightPage *context);

#endif /* Usr_CeilingLightPage__H */

/* end of file */
